    ...
    def _patch_sources(self):
        if self._is_msvc and self.options.x264 and not self.options["libx264"].shared:
            ...
            tools.replace_in_file(
                os.path.join(self._source_subfolder, "libavcodec", "libx264.c"),
    ...
    def build_configure(self):
        with tools.chdir(self._source_subfolder):
            prefix = (
                ...
                if self.settings.os == "Windows"
                else self.package_folder
            )
            args = ["--prefix=%s" % prefix, "--disable-doc", "--disable-programs"]
            ...
            args.append(
                "--enable-postproc" if self.options.postproc else "--disable-postproc"
            )
            ...
            args.append(
                "--enable-libfreetype"
                if self.options.freetype
                else "--disable-libfreetype"
            )
            args.append(
                "--enable-libopenjpeg"
                if self.options.openjpeg
                else "--disable-libopenjpeg"
            )
            args.append(
                "--enable-libopenh264"
                if self.options.openh264
                else "--disable-libopenh264"
            )
            args.append(
                "--enable-libvorbis" if self.options.vorbis else "--disable-libvorbis"
            )
            args.append(
                "--enable-libopus" if self.options.opus else "--disable-libopus"
            )
            ...
            args.append(
                "--enable-libx264" if self.options.x264 else "--disable-libx264"
            )
            args.append(
                "--enable-libx265" if self.options.x265 else "--disable-libx265"
            )
            args.append("--enable-libvpx" if self.options.vpx else "--disable-libvpx")
            args.append(
                "--enable-libmp3lame"
                if self.options.mp3lame
                else "--disable-libmp3lame"
            )
            args.append(
                "--enable-libfdk-aac"
                if self.options.fdk_aac
                else "--disable-libfdk-aac"
            )
            args.append(
                "--enable-libwebp" if self.options.webp else "--disable-libwebp"
            )
            args.append(
                "--enable-openssl" if self.options.openssl else "--disable-openssl"
            )
            ...
            if self.settings.os == "Linux":
                ...
                args.append(
                    "--enable-libpulse" if self.options.pulse else "--disable-libpulse"
                )
                args.append(
                    "--enable-vaapi" if self.options.vaapi else "--disable-vaapi"
                )
                args.append(
                    "--enable-vdpau" if self.options.vdpau else "--disable-vdpau"
                )
                if self.options.xcb:
                    args.extend(
                        [
            ...
            if self.settings.os == "Macos":
                args.append(
                    "--enable-appkit" if self.options.appkit else "--disable-appkit"
                )
                args.append(
                    "--enable-avfoundation"
                    if self.options.avfoundation
                    else "--disable-avfoundation"
                )
                args.append(
                    "--enable-coreimage"
                    if self.options.avfoundation
                    else "--disable-coreimage"
                )
                args.append(
                    "--enable-audiotoolbox"
                    if self.options.audiotoolbox
                    else "--disable-audiotoolbox"
                )
                args.append(
                    "--enable-videotoolbox"
                    if self.options.videotoolbox
                    else "--disable-videotoolbox"
                )
                args.append(
                    "--enable-securetransport"
                    if self.options.securetransport
                    else "--disable-securetransport"
                )

            if self.settings.os == "Windows":
                args.append(
                    "--enable-libmfx" if self.options.qsv else "--disable-libmfx"
                )
    ...
    def package_info(self):
        ...
        if self.settings.os == "Macos":
            ...
            self.cpp_info.system_libs.extend(
                ["ws2_32", "secur32", "shlwapi", "strmiids", "vfw32", "bcrypt"]
            )
